(function () {
    ...
    // === Проверка наличия AndroidJS ===
    if (typeof AndroidJS === 'undefined' ||
        typeof AndroidJS.downloadStart !== 'function') {
        console.log('[Download] AndroidJS not found');
        return;
    }
    ...
    // === Основной диалог выбора раздачи ===
        ...
        ourJackettSearch(card).then(function (list) {
            ...
            Lampa.Select.show({
                ...
                onSelect: function (item) { 
                    pickFileAndStart(item._raw, card, providerId); 
                },
                onBack: function () { 
    ...
    function pickFileAndStart(raw, card, providerId) {
        ...
        ourTsAdd(raw.magnet, raw.title, poster).then(function (hash) {
            ...
            return ourTsFiles(hash).then(function (files) { 
    ...
    function injectDownloadButton(activity) {
        ...
        var $btn = $('<div class="full-start__button selector lampa-download-btn">' +
    ...
    function ensureMenuItem() {
        try {
            if (!listAll().length) { 
                $('.lampa-downloads-menu-item').remove(); 
                return; 
            }
            
            var $menu = $('.menu .menu__list').first();
            if (!$menu.length) return;
            if ($menu.find('.lampa-downloads-menu-item').length) return;
            
            var $item = $('<li class="menu__item selector lampa-downloads-menu-item">' +
                '<div class="menu__ico">' +
                '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">' +
                '<path d="M12 3v13M6 13l6 6 6-6M4 21h16" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>' +
                '</svg></div>' +
    ...
    try {
        ...
        console.log('[Download] component add failed', e); 
    }
