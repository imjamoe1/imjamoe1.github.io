    ...
    function component(object) {
        ...
        this.draw = function(items) {
            ...
            this.getEpisodes(items[0].season, function(episodes) {
                ...
                // ПАТЧ: для фильмов сортируем источники по максимальному доступному качеству (4K → 1440 → 1080).
                // Для сериалов сохраняем оригинальный порядок эпизодов.
                if (!serial) {
                    items.sort(function(a, b) {
                        function maxScore(el) {
                            if (!el || !el.quality || typeof el.quality !== 'object') return 0;
                            var keys = Object.keys(el.quality);
                            if (!keys.length) return 0;
                            return keys.reduce(function(m, k) {
                                var s = qualityScore(k);
                                return s > m ? s : m;
                            }, 0);
                        }
                ...
                items.forEach(function(element, index) {
                    ...
                    if (element.quality) {
                        // ПАТЧ: убираем варианты ниже 1080p и из подписи источника
                    ...
                    // ПАТЧ: показываем максимальное доступное качество как метку источника (4K / 1440p / 1080p)
                    if (element.qualitys && typeof element.qualitys === 'object') {
                        var keys = Object.keys(element.qualitys);
                        if (keys.length) {
                            var best = keys.reduce(function(m, k) {
                                return qualityScore(k) > qualityScore(m) ? k : m;
                            }, keys[0]);
                            var score = qualityScore(best);
                            element.quality = score >= 2160 ? '4K' : (score > 0 ? score + 'p' : best);
                        }
                    }
        ...
        var source = {
            ...
            onSelect: function(params, close) {
                ...
                Lampa.Activity.push({
                    ...
                    title: 'UHD - ' + params.element.title,
                    component: 'UHD',
    ...
    function startPlugin() {
        ...
        var manifst = {
            ...
            onContextLauch: function onContextLauch(object) {
                resetTemplates();
                Lampa.Component.add('UHD', component);
