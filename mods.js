...
(function () {
	...
	var ico = '<svg class="modss-online-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" fill="#9400d3"><path d="M20.331 14.644l-13.794-13.831 17.55 10.075zM2.938 0c-0.813 0.425-1.356 1.2-1.356 2.206v27.581c0 1.006 0.544 1.781 1.356 2.206l16.038-16zM29.512 14.1l-3.681-2.131-4.106 4.031 4.106 4.031 3.756-2.131c1.125-0.893 1.125-2.906-0.075-3.8zM6.538 31.188l17.55-10.075-3.756-3.756z"/></svg>';
	var button = "<div style='position:relative' data-subtitle='modss_v".concat(manifest.version, " (20 Balansers, 16 in VIP)' class='full-start__button selector view--modss_online'>" + ico + "<span>" + this.title + "</span></div>");
...
rating_kp_imdb: function (card) {
    return new Promise(function (resolve, reject) {
        ...
        function searchFilm() {
            ...
            console.log("clean_title", clean_title);
            if (card.imdb_id)
            ...
            if (items && items.length) {
                ...
                if (cards.length == 1 && is_sure && !is_imdb) {
                    ...
                    if (is_sure) {
                        is_sure = false;
                        if (orig) {
                            is_sure |= equalTitle(cards[0].orig_title || cards[0].nameOriginal, orig) ||
                                      equalTitle(cards[0].en_title || cards[0].nameEn, orig) ||
                                      equalTitle(cards[0].title || cards[0].ru_title || cards[0].nameRu, orig);
                        }
                        if (card.title) {
                            is_sure |= equalTitle(cards[0].title || cards[0].ru_title || cards[0].nameRu, card.title) ||
  ...
  function lumex(component, _object) {
    ...
    function lumex_search(api, callback, error) {
      ...
      network["native"](api, success_check, error_check, false, false);
    }
    ...
    this.searchs = function (_object, kinopoisk_id) {
      ...
      var src = API + 'lumex/searchId/';
      src += (object.movie.imdb_id ? encodeURIComponent(object.movie.imdb_id) : '') + '/' + (+object.movie.kinopoisk_id  || +object.movie.kinopoisk_ID ? encodeURIComponent(+object.movie.kinopoisk_id || +object.movie.kinopoisk_ID) : 'null');
   
      lumex_search(src, function (json) {
        if (json && json.id) {
          var src2 = API + 'lumex/findID/';
          src2 += json.id + '/';
          src2 += json.content_type;
    ...
    function getStream(element, call, error) {
      ...
      var prot = Protocol().replace('//', '');
      network.clear();
      network.timeout(10000);
      network["native"](extract.host + element.media.playlist, function (json) {
        var url = json && json.url ? prot + json.url : '';
        if (url) {
          if (url.substr(-5) === '.m3u8') {
            ...
            for (var i = 0; i < qualitiesList.length; i++) {
              ...
              var rpl = prot + json.url.replace('/hls.m3u8', '/' + q + '.mp4');
              if (1080 >= q) qualitys[q + 'p'] = rpl;
            }
            element.url = Object.values(qualitys)[0];
            element.qualitys = qualitys;
          }
          call(element);
    ...
    var headers = Lampa.Platform.is('android') ? {
  	...
  	this.find = function (filmix_id) {
		  ...
		  function end_search(filmix_id) {
  			...
  			network["native"](prox + embed + 'post/' + filmix_id +'?'+ dev_token, function (found) {
  			  if(found) {
            found = {
  ...
  function component(object) {
    ...
    var isVipBal = (Lampa.Storage.get('pro_pub', false) ? 4 : 3);
    ...
    this.proxy = function (name) {
  		...
  		var myprox2 = 'http://cors.cfhttp.top/';
      var proxy_apn = (window.location.protocol === 'https:' ? 'https://' : 'http://') + 'byzkhkgr.deploy.cx/';
  		...
  		if ((need == 'on' || main) && name == 'filmix' && need_url.length == 0) return myprox2;
      ...
      title_prioriry_balanser_descr: {
		...
		Lampa.Settings.listener.follow('open', function (e) {
			...
			if (e.name == 'filmix_param') {
				var ads = ['<div style="padding: 1.5em 2em; padding-top: 10px;">', '<div style="background: #3e3e3e; padding: 1em; border-radius: 0.3em;">', '<div style="font-size: 1em; margin-bottom: 1em; color: #ffd402;">#{info_attention} ⚠</div>', '<div style="line-height: 1.4;">#{info_filmix_descr}</div>', '</div>', '</div>'].join('');
		...
		if (Lampa.Manifest.app_digital >= 177) {
			...
			var balansers_sync = ['lumex', 'fxpro','iremux','hdfilmtr', 'videx', 'kinopub', 'alloha', 'collaps', 'mango', 'filmix', 'zetflix', 'hdr', 'hdrezka', 'uakino', 'eneida', 'videodb', 'hdvb', 'kinotochka', 'kodik', 'anilibria'];
      ...
      Lampa.SettingsApi.addParam({
    	...
    	Lampa.SettingsApi.addParam({
		  ...
		  //Proxy mods
			...
			['Filmix', 'Kinobase', 'Collaps'].forEach(function (itm) {
				Lampa.SettingsApi.addParam({
					component: 'mods_proxy',
					param: {
