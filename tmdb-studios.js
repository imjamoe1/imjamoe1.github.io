(function () {
    ...
    function addLocalization() {
        Lampa.Lang.add({
            ...
            tmdb_networks_plugin_hide: {
                en: 'Information line',
                uk: 'Інформаційний рядок',
                ru: 'Информационная строка'
            },
            tmdb_networks_plugin_descr: {
                en: 'The plugin adds buttons for streaming services, platforms and studios to cards, showing where movies and series were released or sold, making them easier to find',
                uk: 'Плагін додає в картки кнопки стримінгових сервісів, платформ та студій, де виходили або продавалися фільми та серіали, що спрощує їх пошук',
                ru: 'Плагин добавляет в карточки кнопки стриминговых сервисов, платформ и студий, где выходили или продавались фильмы и сериалы, упрощая их поиск'
            },
            platfroms_list: {
                en: 'List',
                uk: 'Перелік',
    ...
    function getProviders(object, callback) {
    ...
    function getStudios(object, callback) {
        if (!object?.card) {
            return callback([]);
        }
        ...
        if (object.card.networks?.length) {
            ...
            studios = object.card.production_companies.filter(function(studio) { 
    ...
    function showNetworkMenu(network, type, element) {
        ...
        Lampa.Select.show({
            ...
            items: [
                { title: Lampa.Lang.translate('tmdb_networks_top'), sort: '', filter: { 'vote_count.gte': 10 } },
                { title: Lampa.Lang.translate('tmdb_networks_new'), sort: dateField + '.desc', filter: { 'vote_count.gte': 10, [dateField + '.lte']: currentDate } }
            ],
            onBack: function() {
            ...
            onSelect: function(action) {
                ...
                if (action.filter && action.filter[dateField + '.lte']) {
                ...
                Lampa.Activity.push({
                    ...
                    page: 1,
                    sort_by: action.sort,
                    filter: Object.assign(filter, isTv ? { with_networks: network.id } : { with_companies: network.id })
                });
            }
        });
    }
    ...
    function addStudioButton(render, networks, type, cardId) {
        ...
        var networksContainer = $('.tmdb-networks', render);
        if (!networksContainer.length) {
            setTimeout(function() {
                addStudioButton(render, networks, type, cardId);
            }, 200);
            return;
        }
        
        var tagsContainer = networksContainer.find('.full-descr__tags');
        ...
        var platformsStatic = tagsContainer.find('.platforms-static');
        ...
        var studiosStatic = $('<div class="studios-static">' +
            '<span class="studios-static__text">' + Lampa.Lang.translate('tmdb_networks_studios') + '</span>' +
        '</div>');
    ...
    function renderNetworks() {
        ...
        if (!settings.hide_tmdb_networks) return;

        getProviders(object, function(networks) {
            if (networks.length == 0) return;
            ...
            var platformsItem = $('<div class="platforms-static">' +
                '<span class="platforms-static__text">' + Lampa.Lang.translate('tmdb_networks_plugin_platforms') + '</span>' +
            '</div>');
    ...
    function addSettings() {
        ...
        Lampa.SettingsApi.addParam({
            ...
            onChange: function(value) {
                settings.hide_tmdb_networks = value;
                Lampa.Storage.set('hide_tmdb_networks', value);
                toggleTmdbNetworks();
            }
        });
    ...
    function startPlugin() {
        if (window.tmdb_networks_combined) {
            return;
        }
        window.tmdb_networks_combined = true;

        $('<style>').prop('type', 'text/css').html(
            '.tmdb-networks { margin-top: -4.7em; } ' +
