(function () {
  ...
  function startPlugin() {
    ...
    `;
    Lampa.Template.add("cardify_css", a);
    $("body").append(Lampa.Template.get("cardify_css", {}, true));
    var b = `<svg width="36" height="28" viewBox="0 0 36 28" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    Lampa.SettingsApi.addComponent(c);
    ...
    function d(a) {
      if (a.videos && a.videos.results.length) {
        var b = [];
        a.videos.results.forEach(function (a) {
          b.push({
            title: Lampa.Utils.shortText(a.name, 50),
            id: a.key,
            code: a.iso_639_1,
            time: new Date(a.published_at).getTime(),
            url: "https://www.youtube.com/watch?v=" + a.key,
            img: "https://img.youtube.com/vi/" + a.key + "/default.jpg"
          });
        });
        b.sort(function (a, b) {
          if (a.time > b.time) {
        ...
        var c = b.filter(function (a) {
          return a.code == Lampa.Storage.field("tmdb_lang");
        });
        var d = b.filter(function (a) {
          return a.code == "en" && c.indexOf(a) == -1;
        });
        var e = [];
        if (c.length) {
          e = e.concat(c);
    ...
    G.get(M.de([102, 117, 108, 108]), function (a) {
      if (M.co(a)) {
        G.skodf(a);
        if (!y.cases()[y.stor()].field("cardify_run_trailers")) {
          return;
        }
        var b = G.vjsk(d(a.data));
        if (y.cases().Manifest.app_digital >= 220) {
          if (y.cases().Activity.active().activity === a.object.activity) {
            if (b) {
              new o(a.object, b);
            }
          } else {
            var c = function d(c) {
              if (c.type == M.de([115, 116, 97, 114, 116]) && c.object.activity === a.object.activity && !a.object.activity.trailer_ready) {
                y.cases()[D()].remove("activity", d);
                if (b) {
                  new o(a.object, b);
                }
              }
  ...
  if (G.go) {
    ...
    G.get(M.de([97, 112, 112]), function (a) {
      if (M.re(a)) {
        N();
      }
    });
