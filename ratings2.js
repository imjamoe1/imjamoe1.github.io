(function() {
    ...
    // Время кэширования
    var CACHE_TIME = 24 * 60 * 60 * 1000; // 24 часа
    var Q_CACHE_TIME = 7 * 24 * 60 * 60 * 1000; // 7 дней
    
    // Ключи API
    var OMDB_API_KEYS = (window.RATINGS_PLUGIN_TOKENS && window.RATINGS_PLUGIN_TOKENS.OMDB_API_KEYS) || ['18a1eec9', 'd3a7a896'];
    var KP_API_KEYS   = (window.RATINGS_PLUGIN_TOKENS && window.RATINGS_PLUGIN_TOKENS.KP_API_KEYS)   || ['ae8d6b29-b4ea-4f44-ad64-e99cb243289a'];
    
    // Прокси для Кинопоиска (из второго плагина)
    ...
    // Названия кэшей
    var OMDB_CACHE = 'maxsm_ratings_omdb_cache_v2';
    var KP_CACHE = 'maxsm_ratings_kp_cache_v2';
    var QUALITY_CACHE = 'maxsm_ratings_quality_cache_v2';
    var ID_MAPPING_CACHE = 'maxsm_ratings_id_mapping_cache_v2';
    ...
    // ========================================
    // 3. ФУНКЦИИ КЭШИРОВАНИЯ (оптимизированные)
    // ========================================
    
    function getCache(cacheKey, key) {
        var cache = Lampa.Storage.cache(cacheKey, 1000, {});
        var item = cache[key];
        return item && (Date.now() - item.timestamp < CACHE_TIME) ? item : null;
    }
    
    function setCache(cacheKey, key, data) {
        var cache = Lampa.Storage.cache(cacheKey, 1000, {});
        cache[key] = {
            data: data,
            timestamp: Date.now()
        };
        Lampa.Storage.set(cacheKey, cache);
    }
    
    function getOmdbCache(key) {
        var cache = Lampa.Storage.cache(OMDB_CACHE, 1000, {});
        var item = cache[key];
    ...
    function getKPCache(key) {
        var cache = Lampa.Storage.cache(KP_CACHE, 1000, {});
        var item = cache[key];
        return item && (Date.now() - item.timestamp < CACHE_TIME) ? item.data : null;
    }
    ...
    function getQualityCache(key) {
        var cache = Lampa.Storage.cache(QUALITY_CACHE, 1000, {});
        var item = cache[key];
        return item && (Date.now() - item.timestamp < Q_CACHE_TIME) ? item.data : null;
    }
    ...
    // ========================================
    // 4. ЗАПРОСЫ К API (параллельные)
    // ========================================
    
    // 4.1. Кинопоиск (из второго плагина)
    async function fetchKPRatings(movieData) {
        if (!movieData || !movieData.id) return null;
        
        var tmdb_id = movieData.id;
        
        // Проверяем кэш
        var cached = getKPCache(tmdb_id);
        if (cached) {
            if (C_LOGGING) console.log("KP: из кэша", tmdb_id);
            return cached;
        }
        ...
        var imdb_id = movieData.imdb_id;
        
        var clean_title = kpCleanTitle(title);
        var network = new Lampa.Reguest();
        
        try {
            // Сначала пробуем по IMDb ID
            if (imdb_id) {
                var imdbSearchUrl = KP_CONFIG.api_url + 'api/v2.2/films?imdbId=' + encodeURIComponent(imdb_id);
                
                const imdbResponse = await new Promise((resolve, reject) => {
                    network.clear();
                    network.timeout(KP_CONFIG.timeout);
                    network.native(imdbSearchUrl, function(data) {
                        resolve(data);
                    }, function(error) {
                        reject(error);
                    }, false, { 
                        headers: { 'X-API-KEY': KP_CONFIG.api_key } 
                    });
                });
                
                if (imdbResponse && imdbResponse.items && imdbResponse.items.length > 0) {
                    const film = imdbResponse.items[0];
                    const kpRating = await fetchKpRating(film.kinopoiskId || film.filmId);
                    const result = { kp: kpRating, filmId: film.kinopoiskId || film.filmId };
                    saveKPCache(tmdb_id, result);
                    return result;
                }
            }
            
            // Поиск по названию
            var searchUrl = KP_CONFIG.api_url + 'api/v2.1/films/search-by-keyword?keyword=' + encodeURIComponent(clean_title) + '&page=1';
            if (search_year) {
                searchUrl += '&yearFrom=' + search_year + '&yearTo=' + search_year;
            }
            ...
            if (!searchResponse || !searchResponse.films || searchResponse.films.length === 0) {
                return null;
            }
            ...
            searchResponse.films.forEach(film => {
                let score = 0;
                
                if (orig_title) {
                    if (equalTitle(film.nameOriginal || film.nameEn, orig_title)) {
                        score += 3;
                    }
                }
                ...
                if (search_year && film.year === search_year) {
                    score += 1;
                } else if (search_year && film.year && Math.abs(film.year - search_year) <= 1) {
                    score += 0.5;
                }
            ...
            if (!bestMatch && searchResponse.films.length > 0) {
                bestMatch = searchResponse.films[0];
            }
            ...
            if (C_LOGGING) console.error('KP error:', error);
            return null;
        }
    }
    ...
    async function fetchKpRating(filmId) {
        ...
        try {
            ...
            if (xmlResponse && typeof xmlResponse === 'string') {
                ...
                var kpRatingElement = xmlDoc.querySelector('kp_rating');
                
                if (kpRatingElement && kpRatingElement.textContent) {
                    var rating = parseFloat(kpRatingElement.textContent);
        ...
        try {
            ...
            if (apiResponse && apiResponse.ratingKinopoisk) {
                var rating = parseFloat(apiResponse.ratingKinopoisk);
                if (!isNaN(rating) && rating > 0) {
                    return rating.toFixed(1);
                }
            }
        } catch (e) {
            if (C_LOGGING) console.error('KP API error:', e);
        }
        
        return '0.0';
    }
    
    // 4.2. OMDB (IMDb, Rotten Tomatoes, Metacritic)
    function fetchOmdbRatings(card, callback) {
        ...
        var cacheKey = card.type + '_' + card.imdb_id;
        var cached = getOmdbCache(cacheKey);
        if (cached) {
            if (C_LOGGING) console.log("OMDB: из кэша", cacheKey);
            callback(cached);
            return;
        }
    ...
    function parseAwards(awardsText) {
        if (typeof awardsText !== 'string') return { oscars: 0, awards: 0 };
    ...
    function getImdbIdFromTmdb(tmdbId, type, callback) {
        ...
        if (cached) {
            callback(cached.imdb_id);
            return;
        }
        
        var cleanType = type === 'movie' ? 'movie' : 'tv';
        var url = Lampa.TMDB.api(cleanType + '/' + tmdbId + '/external_ids?api_key=' + Lampa.TMDB.key());
        
        new Lampa.Reguest().silent(url, function(data) {
            if (data && data.imdb_id) {
                setCache(ID_MAPPING_CACHE, cacheKey, { imdb_id: data.imdb_id });
    ...
    function getBestReleaseFromJacred(normalizedCard, callback) {
        ...
        var stopWords = ['camrip', 'камрип', 'ts', 'telecine', 'telesync', 'upscale'];
        ...
        function isScreenCopy(title) {
            ...
            for (var i = 0; i < stopWords.length; i++) {
                if (lower.indexOf(stopWords[i]) !== -1) {
                    return true;
                }
            }
            return false;
        ...
        new Lampa.Reguest().silent(apiUrl, function(response) {
            ...
            try {
                ...
                if (bestTorrent) {
                    callback({ 
                        quality: translateQuality(bestTorrent.quality), 
    ...
    // ========================================
    // 6. ОСНОВНАЯ ФУНКЦИЯ (ПАРАЛЛЕЛЬНАЯ ЗАГРУЗКА)
    // ========================================
    ...
    function fetchAdditionalRatings(card, render) {
        ...
        if (C_LOGGING) console.log("MAXSM-RATINGS: старт для", localCurrentCard);
        ...
        // Считаем сколько запросов будем делать
        if (normalizedCard.imdb_id) totalRequests++;
        totalRequests++; // KP всегда
        ...
        // ЗАПРОС 1: Кинопоиск (параллельно)
        fetchKPRatings({
            id: normalizedCard.id,
            title: normalizedCard.title,
            original_title: normalizedCard.original_title,
            release_date: normalizedCard.release_date,
            first_air_date: normalizedCard.release_date,
            imdb_id: normalizedCard.imdb_id
        }).then(function(kpResult) {
            if (kpResult) {
                ratingsData.kp = kpResult.kp;
                if (C_LOGGING) console.log("KP: получено", kpResult.kp);
            }
            checkComplete();
        }).catch(function() {
            checkComplete();
        ...
        // ЗАПРОС 2: OMDB (если есть IMDb ID)
        if (normalizedCard.imdb_id) {
            fetchOmdbRatings(normalizedCard, function(omdbData) {
                if (omdbData) {
                    ...
                    if (C_LOGGING) console.log("OMDB: получено");
                }
                checkComplete();
            });
        } else {
            // Если нет IMDb ID - пробуем получить его через TMDB
            getImdbIdFromTmdb(normalizedCard.id, normalizedCard.type, function(imdbId) {
                if (imdbId) {
                    normalizedCard.imdb_id = imdbId;
                    fetchOmdbRatings(normalizedCard, function(omdbData) {
                        if (omdbData) {
                            ratingsData.rt = omdbData.rt;
                            ratingsData.mc = omdbData.mc;
        ...
        // ЗАПРОС 3: Качество (в фоне, не блокирует)
        if (localStorage.getItem('maxsm_ratings_quality') === 'true') {
            ...
            if (cacheQualityData) {
                if (Q_LOGGING) console.log("Качество: из кэша");
        ...
        function updateUI() {
            // Вставляем рейтинги
            insertRatings(ratingsData, localCurrentCard, render);
            ...
            if (mode !== 2) {
                calculateAverageRating(ratingsData, localCurrentCard, render);
            }
            ...
            if (C_LOGGING) console.log("MAXSM-RATINGS: готово для", localCurrentCard);
        }
    }
    
    // ========================================
    // 7. ФУНКЦИИ ОБНОВЛЕНИЯ UI
    // ========================================
    
    function insertRatings(ratings, localCurrentCard, render) {
        if (!render) return;
        ...
        // RT (Rotten Tomatoes)
        if (showRT && ratings.rt && !isNaN(ratings.rt) && !$('.rate--rt', rateLine).length) {
            var rtElement = $(
        ...
        // MC (Metacritic)
        if (showMC && ratings.mc && !isNaN(ratings.mc) && !$('.rate--mc', rateLine).length) {
        ...
        // Оскары
        if (showAwards && ratings.oscars && ratings.oscars > 0 && !$('.rate--oscars', rateLine).length) {
            var oscarsElement = $(
                '<div class="full-start__rate rate--oscars' + (showColors ? ' rate--gold' : '') + '">' +
        ...
        // Другие награды
        if (showAwards && ratings.awards && ratings.awards > 0 && !$('.rate--awards', rateLine).length) {
            var awardsElement = $(
                '<div class="full-start__rate rate--awards' + (showColors ? ' rate--gold' : '') + '">' +
    ...
    function updateHiddenElements(ratings, localCurrentCard, render) {
        ...
        if (pgElement.length && ratings.ageRating) {
            ...
            if (isValid && showPg) {
                pgElement.removeClass('hide').text(ratings.ageRating);
            } else {
    ...
    function calculateAverageRating(ratings, localCurrentCard, render) {
        if (!render) return;
        ...
        var ratingValues = {
            ...
            imdb: parseFloat(ratings.imdb) || 0,
            kp: parseFloat(ratings.kp) || 0,
            mc: (parseFloat(ratings.mc) || 0) / 10,
            rt: (parseFloat(ratings.rt) || 0) / 10
        };
        ...
        for (var key in ratingValues) {
            if (ratingValues.hasOwnProperty(key) && !isNaN(ratingValues[key]) && ratingValues[key] > 0) {
                weightedSum += ratingValues[key] * WEIGHTS[key];
        ...
        if (totalWeight > 0 && (ratingsCount > 1 || mode === 1)) {
            ...
            var avgLabel = mode === 1 ? 
                Lampa.Lang.translate("maxsm_ratings_avg_simple") : 
                Lampa.Lang.translate("maxsm_ratings_avg");
            
            var avgElement = $(
                '<div class="full-start__rate rate--avg' + (showColors ? ' ' + colorClass : '') + '">' +
    ...
    // ========================================
    // 10. ЗАМЕНА BYLAMPA НА ЗВЕЗДОЧКУ
    // ========================================
    ...
    function insertIcons(localCurrentCard, render) {
        ...
        var iconMap = {
            ...
            'rate--kp': kp_svg,
            'rate--tmdb': tmdb_svg,
            'rate--oscars': oscars_svg,
            'rate--awards': awards_svg,
            'rate--rt': rt_svg,
            'rate--mc': mc_svg,
            'rate--avg': avg_svg,
            'rate--lampa': lampa_svg
        };
        
        for (var className in iconMap) {
            if (iconMap.hasOwnProperty(className)) {
                var element = $('.' + className, render);
                if (element.length) {
                    var sourceName = element.find('.source--name');
                    if (sourceName.length) {
                        sourceName.html(iconMap[className]).addClass('rate--icon');
                    }
                }
            }
        }
    ...
    var loadingStyles = `
        <style id="maxsm_ratings_loading_animation">
    ...
    Lampa.Lang.add({
        ...
        maxsm_ratings_pg: { ru: 'Возрастной рейтинг', en: 'Age rating' },
        maxsm_ratings_status: { ru: 'Статус внутри карточек', en: 'Status inside cards' },
    ...
    function initializeSettings() {
        Lampa.SettingsApi.addComponent({
            ...
            icon: '<!-- ВСТАВЬТЕ SVG ЗВЕЗДЫ ЗДЕСЬ -->'
        });
    ...
    function startPlugin() {
        ...
        // Добавляем стили
        $('body').append(loadingStyles);
        $('body').append(ratingStyles);
