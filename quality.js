(function () {
    ...
    var Q_LOGGING = true; // Логирование качества
    ...
    function getBestReleaseFromJacred(normalizedCard, cardId, callback) {
        ...
        function searchJacredApi(searchTitle, searchYear, exactMatch, strategyName, apiCallback) {
            ...
            fetchWithProxy(apiUrl, cardId, function(error, responseText) {
                clearTimeout(timeoutId);
                if (error) {
                    console.error("MAXSM-RATINGS", "card: " + cardId + ", quality: JacRed: " + strategyName + " запрос не удался:", error);
                    apiCallback(null);
                    return;
                }
                if (!responseText) {
    ...
    function updateCards(cards) {
        for (var i = 0; i < cards.length; i++) {
            ...
            (function (currentCard) {
                ...
                if (cacheQualityData) {
                    if (Q_LOGGING) console.log("MAXSM-RATINGS", "card: " + localCurrentCard + ", качество: Get Quality data from cache");
