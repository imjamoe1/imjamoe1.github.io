...
(()=>{
  ...
  A=(a,e)=>{for(var t in e||(e={}))Fe.call(e,t)&&se(a,t,e[t]);if(ne)for(var t of ne(e))Ye.call(e,t)&&se(a,t,e[t]);return a},
  D=(a,e)=>Ue(a,ze(e));
  var l=(a,e,t)=>new Promise((o,n)=>{var r=c=>{try{i(t.next(c))}catch(g){n(g)}},s=c=>{try{i(t.throw(c))}catch(g){n(g)}},i=c=>c.done?o(c.value):Promise.resolve(c.value).then(r,s);i((t=t.apply(a,e)).next())});
  ...
  var re={
    ...
    "background-worker.retry":{ru:"Повторная попытка подключения...",en:"Retrying connection..."},
    "background-worker.no-connection":{ru:"Нет подключения к серверу. Проверьте настройки.",en:"No connection to server. Check settings."}
  };
  ...
  w=class{
    ...
    static getMovie(e){if(!e)return null;let t=this.data.torrents.filter(o=>o.id===e);return t.length>0?t.reduce((o,n)=>o.percentDone>=n.percentDone?o:n):null}
    static getByHash(e){var t;return(t=this.data.torrents.find(o=>o.hash===e))!=null?t:null}
    static ensureMovie(e){let t=this.data.torrents.filter(o=>o.externalId===e.externalId);return t.length>0?t.reduce((o,n)=>o.percentDone>=n.percentDone?o:n):e}
    static setData(e){return l(this,null,function*(){this.data=e,Lampa.Storage.set(ie,this.data)})}
  };
  w.data=Lampa.Storage.get(ie,{torrents:[],info:{freeSpace:0}});
  
  var isAndroidTV = typeof window !== 'undefined' && window.navigator && 
  ...
  b=class{
    static getMovieInfo(e){if(!e.id)return null;let t=`${e.type}_${e.id}`;return this.memoryCache[t]?this.memoryCache[t]:(this.requestedIds.has(t)||(this.requestedIds.add(t),this.loadContentInfo(e.id,e.type).then(o=>{o?(this.memoryCache[t]=o,this.diskCache[t]=o,Lampa.Storage.set(pe,this.diskCache)):this.requestedIds.delete(t)}).catch(()=>{this.requestedIds.delete(t)})),this.diskCache[t]||null)}
    static loadContentInfo(e,t,o=!0){return l(this,null,function*(){let n=Lampa.Storage.field("tmdb_lang")||Lampa.Storage.field("language")||"ru",r=Lampa.Utils.addUrlComponent(Lampa.TMDB.api(`${t}/${e}?email=`),`api_key=${Lampa.TMDB.key()}&language=${n}&certification_country=ru&certification.lte=18`);try{let s=yield fetch(r);if(s.ok){let i=yield s.json();if(i!=null&&i.title||i!=null&&i.name)return i}else if(o){h(`Failed to load '${t}_${e}', status: ${s.status}. Trying fallback type.`);let i=t==="movie"?"tv":"movie";return yield this.loadContentInfo(e,i,!1)}}catch(s){ce(`Failed to load ${t} info for id ${e}:`,s)}return null})}
  };
  b.requestedIds=new Set,b.diskCache=Lampa.Storage.get(pe,{}),b.memoryCache={};
  ...
  function me(a){switch(a){case 0:return p.STOPPED;case 1:return p.CHECK_PENDING;case 2:return p.CHECKING;case 3:return p.DOWNLOAD_PENDING;case 4:return p.DOWNLOADING;case 5:return p.SEED_PENDING;case 6:return p.SEEDING;default:return p.UNKNOWN}}
  
  function ue(a){switch(a){case"allocating":return p.ALLOCATING;case"checkingDL":case"checkingUP":case"checkingResumeData":return p.CHECKING;case"queuedDL":return p.DOWNLOAD_PENDING;case"queuedUP":return p.SEED_PENDING;case"downloading":case"forcedMetaDL":return p.DOWNLOADING;case"uploading":case"forcedUP":return p.SEEDING;case"pausedDL":case"pausedUP":case"stoppedDL":case"stoppedUP":return p.STOPPED;case"stalledDL":case"stalledUP":return p.STALLED;case"missingFiles":return p.ISOLATED;case"moving":return p.MOVING;case"error":return p.ERROR;case"metaDL":case"forcedDL":return p.INITIALIZATION;default:return p.UNKNOWN}}
  
  function L(a,e=2){if(a===0)return"0";let t=1024,o=e<0?0:e,n=Math.floor(Math.log(a)/Math.log(t));return parseFloat((a/Math.pow(t,n)).toFixed(o))+" "+Lampa.Lang.translate(`download-card.size.${n}`)}
  
  function Be(a){let e=Lampa.Lang.translate("download-card.time.3");return`${L(a)}/${e}`}
  
  function Ve(a){let e=Math.floor(a/86400),t=Math.floor(a%86400/3600),o=Math.floor(a%3600/60),n=Math.floor(a%60);return[e,t,o,n].map((s,i)=>s?s+Lampa.Lang.translate(`download-card.time.${i}`):null).filter(Boolean).slice(0,2).join(" ")}
  
  function je(a){let e=new Date(a||"");return isNaN(e.getTime())?"":e.getFullYear()}
  ...
  function S(a,e,t,o){e=w.ensureMovie(e),Lampa.Select.show({title:Lampa.Lang.translate("actions.title"),items:[{title:Lampa.Lang.translate("actions.open"),onSelect(){return l(this,null,function*(){G(a,e,t)})}},...a==="downloads-tab"&&e.id?[{title:Lampa.Lang.translate("actions.open-card"),onSelect(){return l(this,null,function*(){Lampa.Activity.push({component:"full",id:e.id,method:e.type,card:e})})}}]:[],{title:e.status===p.STOPPED?Lampa.Lang.translate("actions.resume"):Lampa.Lang.translate("actions.pause"),onSelect(){W(e),Lampa.Controller.toggle(a)}},{title:Lampa.Lang.translate("actions.hide"),onSelect(){m.getClient().hideTorrent(e),o==null||o(e),Lampa.Controller.toggle(a)}},{title:Lampa.Lang.translate("actions.delete"),subtitle:Lampa.Lang.translate("actions.delete-with-file"),onSelect(){m.getClient().removeTorrent(e,!0),o==null||o(e),Lampa.Controller.toggle(a)}},{title:Lampa.Lang.translate("actions.delete-torrent"),subtitle:Lampa.Lang.translate("actions.delete-torrent-keep-file"),onSelect(){m.getClient().removeTorrent(e,!1),o==null||o(e),Lampa.Controller.toggle(a)}}],onBack:function(){Lampa.Controller.toggle(a)}})}
  ...
  function V(a,e){let t=$(Lampa.Template.get("download-card",v(a)));$(".full-start-new__right").append(t),t.on("hover:enter",()=>{C("full_start",a,(e==null?void 0:e.title)||(e==null?void 0:e.original_title))}),t.on("hover:long",()=>{S("full_start",a,(e==null?void 0:e.title)||(e==null?void 0:e.original_title))})}
  
  function we(a){let e=v(a),t=document.getElementById(`download-card-${e.id}`);if(t){for(let o in e){let n=t.querySelector(`[data-key="${o}"]`);n&&(n.textContent=e[o])}t.querySelector(".download-card__progress-bar-progress").setAttribute("style",`width: ${e.percent};`)}}
  
  function he(){Lampa.Template.add("download-card",de),$("body").append(`<style>${le}</style>`),Lampa.Listener.follow("full",a=>{if(a.type==="complite"){let e=w.getMovie(a.data.movie.id);e&&V(e,a.data.movie)}})}
  ...
  function Le(){Lampa.Template.add("download-circle",_e),$("body").append(`<style>${be}</style>`),Lampa.Listener.follow("line",a=>{var e,t;if(a.type==="append")for(let o of a.items)(e=o==null?void 0:o.data)!=null&&e.id&&Je((t=o==null?void 0:o.data)==null?void 0:t.id,o.card)})}
  ...
  function tt(a){let e=new Map;return a.forEach((t,o)=>{let n=t.id>0?String(t.id):`solo_${t.externalId}`;e.has(n)||e.set(n,{torrents:[],lastIndex:o});let r=e.get(n);r.torrents.push(t),r.lastIndex=Math.max(r.lastIndex,o)}),[...e.values()].sort((t,o)=>t.lastIndex-o.lastIndex).map(t=>[...t.torrents].sort((o,n)=>n.totalSize-o.totalSize))}
  
  var j=class{
    constructor(){this.html=$("<div></div>");this.lastFocusedElement=null}
    create(){m.isConnected||_.start(),this.scroll=new Lampa.Scroll({mask:!0,over:!0,step:200});let e=w.getData(),t=m.isConnected?Lampa.Lang.translate("downloads-tab.connected")+" ("+m.getClient().url+")":Lampa.Lang.translate("downloads-tab.disconnected"),o=$(Lampa.Template.get("downloads-tab",{server:t,freeSpace:Lampa.Lang.translate("downloads-tab.freespace")+L(e.info.freeSpace)}));this.$rows=o.find(".downloads-tab__rows");let n=window.innerWidth<=window.innerHeight,r=tt(e.torrents);if(n)r.forEach(s=>this.$rows.append(this.buildElement(s)));else{let s=[$('<div class="downloads-tab__col"></div>'),$('<div class="downloads-tab__col"></div>')];this.$rows.append(s[0]).append(s[1]);let i=[0,0];r.forEach(c=>{let g=i[0]<=i[1]?0:1;i[g]+=c.length,s[g].append(this.buildElement(c))})}this.scroll.minus(),this.scroll.append(o.get(0)),this.html.append(this.scroll.render())}
    buildElement(e){let t=e.map((o,n)=>{let r=v(o);return $(Lampa.Template.get(n===0?"downloads-row":"downloads-mini-row",r)).on("hover:focus",s=>{this.lastFocusedElement=s.currentTarget,this.scroll.update(s.currentTarget,!0)}).on("hover:enter",()=>C("downloads-tab",o,void 0,s=>this.removeTorrentFromUI(s))).on("hover:long",()=>S("downloads-tab",o,void 0,s=>this.removeTorrentFromUI(s)))});if(e.length>1){let o=$('<div class="downloads-tab__group"></div>');return t.forEach(n=>o.append(n)),o}return t[0]}
    removeTorrentFromUI(e){let t=`${e.id}_${e.externalId}`,o=this.html.find(`.downloads-tab__item[data-id="${t}"]`);if(!o.length)return;let n=o.closest(".downloads-tab__group"),r=n.length>0,s=r&&n.find(".downloads-tab__item").first().is(o),i=o.nextAll(".downloads-tab__item").first();if(i.length||(i=o.prevAll(".downloads-tab__item").first()),!i.length){let c=r?n:o;i=c.nextAll(".downloads-tab__item, .downloads-tab__group").first(),i.length||(i=c.prevAll(".downloads-tab__item, .downloads-tab__group").first())}if(o.remove(),r){let c=n.find(".downloads-tab__item");if(c.length===0)n.remove();else if(s){let g=c.first(),f=g.attr("data-id")||"",E=f.substring(f.indexOf("_")+1),ae=w.getData().torrents.find(Y=>String(Y.externalId)===E);if(ae){let Y=v(ae),oe=$(Lampa.Template.get("downloads-row",Y));g.attr("class",oe.attr("class")||""),g.empty().append(oe.contents())}}}if(Lampa.Controller.collectionSet(this.scroll.render()),i!=null&&i.length){let c=i.is(".downloads-tab__item")?i.get(0):i.find(".downloads-tab__item").first().get(0);c&&(Lampa.Controller.collectionFocus(c,this.scroll.render()),this.lastFocusedElement=c)}}render(e=!1){return this.html}start(){Lampa.Controller.add("downloads-tab",{toggle:()=>{var e;Lampa.Controller.collectionSet(this.scroll.render()),Lampa.Controller.collectionFocus((e=this.lastFocusedElement)!=null?e:!1,this.scroll.render())},left:()=>Navigator.canmove("left")?Navigator.move("left"):Lampa.Controller.toggle("menu"),right:()=>Navigator.move("right"),up:()=>Navigator.canmove("up")?Navigator.move("up"):Lampa.Controller.toggle("head"),down:()=>Navigator.canmove("down")&&Navigator.move("down"),back:()=>Lampa.Activity.backward()}),Lampa.Controller.toggle("downloads-tab")}build(e){}bind(e){}empty(){}next(){}append(e,t){}limit(){}refresh(){}pause(){}stop(){}destroy(){this.scroll.destroy(),this.html.remove()}
  };
  
  function Ae(a){let e=v(a),t=$(document).find(`.downloads-tab__item[data-id="${e.id}"]`);t.length&&(t.removeClass("downloading completed paused").addClass(e.status),t.find(".downloads-tab__progress-fill").css("width",e.percent),t.find(".downloads-tab__poster").css("background-image",`url(${e.poster})`),Object.keys(e).forEach(o=>{t.find(`[data-field="${o}"]`).each(function(){$(this).text(e[o])})}))}
  
  function De(){Lampa.Template.add("menu-button",Ee),Lampa.Template.add("downloads-row",Se),Lampa.Template.add("downloads-mini-row",Te),Lampa.Template.add("downloads-tab",Ie),$("body").append(`<style>${xe}</style>`),Lampa.Component.add("downloads-tab",j);let a=Lampa.Lang.translate("downloads"),e=$(Lampa.Template.get("menu-button",{icon:y,text:a}));e.on("hover:enter",function(){Lampa.Activity.push({url:"",title:a,component:"downloads-tab",page:1})}),$(".menu .menu__list").eq(0).append(e)}
  ...
  function M(a){let e=(a.title||a.name).trim(),t=a.release_year||(a.release_date?a.release_date.slice(0,4):"")||(a.first_air_date?a.first_air_date.slice(0,4):""),o="";return Lampa.Storage.field(te)&&(o+=`/${ke(a)?"tv":"movie"}`),o+=`/${e}`,Lampa.Storage.field(N)&&t&&(o+=` (${t})`),Lampa.Storage.field(k)&&(o+=` [tmdbid-${a.id}]`),o}
  ...
  var F=class{
    ...
    getData(){return l(this,null,function*(){return{torrents:yield this.getTorrents(),info:{freeSpace:0}}})}
  };
