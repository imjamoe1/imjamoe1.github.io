(function () {
    ...
    if (typeof AndroidJS === 'undefined' || typeof AndroidJS.downloadStart !== 'function') {
		...
		// Встроенные дефолты «Скачать»: — берём свой TorrServer и jac.red-парсер.
		//
		// В настройки Lampa отсюда не попадает ничего: подмены
		// `Storage.field` больше нет, и этими значениями
		// пользуется только собственный конвейер скачивания. Свой
		// TorrServer настраивается в Lampa как обычно, и мы туда
		// не лезем.
		var DEFAULTS = {
			torrserver_url: 'http://free.torrservera.net:7788',
			torrserver_login: 'ts',
			torrserver_password: 'ts',
			// Встроенный TS требует Basic-Auth, поэтому флаг
		...
		// не подмешиваем свои значения в его настройки. «Скачать»
		// имеет собственный self-contained pipeline: Jackett → torrent
		// файл → POST /torrents на TorrServer → downloadStart —
		// см. lampaDownload* функции ниже. Никаких обёрток над
		// Lampa.Storage больше нет.
		//
		// Единственное исключение — Player.play hook (в блоке ниже),
		// но он срабатывает только когда активна activity с
		// флагом __lampaDownloadMode, поэтому обычный флоу Lampa
		// не задевает.
		...
		function injectDownloadButton(activity) {
			...
			if (!$buttons.length) {
				console.log('[Lampa downloads] injectBtn: buttons container not found; ' +
		...
		// Собственный TorrServer / парсер — используем ТОЛЬКО в
		...
		// Простой picker: JacRed → Lampa.Select → выбор файла →
		...
		function DownloadsComponent(object) {
			...
			// числе карточек нижние не видны: div сам по себе
			...
			function render() {
				...
				activeCards = items.map(function (it) {
					...
					var $card = $(
						'<div class="card selector lampa-download-card" ' +
						'style="width:14em">' +
			...
			function playStream(entry, useInner) {
				...
				// Basic-Auth, что и downloader — без него сервер
			...
			function refresh() {
				...
				peers.forEach(function (peer) {
					...
					$.ajax({
						...
						console.log('[Lampa share] failed ' + listUrl +
