(function () {
  ...
  var Defined = {
    api: 'lampac2015vip',
    localhost: 'http://lampa.isroot.in/',
    apn: ''
  };
...
var hostkey = 'http://lampa.isroot.in'.replace('http://', '').replace('https://', '');
...
window.rch_nws[hostkey].typeInvoke = function rchtypeInvoke(host, call) {
  if (!window.rch_nws[hostkey].startTypeInvoke) {
    ...
    if (Lampa.Platform.is('android') || Lampa.Platform.is('tizen')) check(true);
      ...
      net.silent('http://lampa.isroot.in'.indexOf(location.host) >= 0 ? 'https://github.com/' : host + '/cors/check', function() {
        check(true);
      }, function() {
        check(false);
      }, false, {
...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  window.rch_nws[hostkey].typeInvoke('http://lampa.isroot.in', function() {
    ...
    client.on("RchClient", function(rchId, url, data, headers, returnHeaders) {
	  ...
	  function sendResult(uri, html) {
	    $.ajax({
	      url: 'http://lampa.isroot.in/rch/' + uri + '?id=' + rchId,
	      type: 'POST',
	      data: html,
	      async: true,
	      cache: false,
  ...
  function rchRun(json, call) {
    if (typeof NativeWsClient == 'undefined') {
	  var server_url = atob('aHR0cHM6Ly9ha3Rlci1ibGFjay5jb20='); 
	  var currentCorsProxy = atob('aHR0cHM6Ly9lbmhhbmNlZC5qZWV0Ym90LmNjLw==');
	  var url = "http://lampa.isroot.in/js/nws-client-es5.js?v21042026";
	  var old_server_url = Defined.localhost.replace(/\/+$/, '');
	  
	  url = url.indexOf(old_server_url) === 0 ? server_url.replace(/\/+$/, '') + url.substring(old_server_url.length) : url;

	  if (currentCorsProxy !== 'None'){
		var lastChar = currentCorsProxy.slice(-1);
		...
		if (lastChar === '=') {
			url = currentCorsProxy + encodeURIComponent(url);
		} else {
			url = currentCorsProxy + url;
		}				  
	  }
	  
      Lampa.Utils.putScript([url], function() {}, false, function() {
        rchInvoke(json, call);
      }, true);
    } else {
      rchInvoke(json, call);
  ...
  function setParam(url, key, value){
	if (url.indexOf(key + '=') === -1){
		...
		return url.replace(new RegExp(key + '=[^&]+'), key + '=' + encodeURIComponent(value));
	}
  }
  ...
  function account(url) {
    ...
    if (url.indexOf('uid=') == -1) { var uid = Lampa.Storage.get('lampac_unic_id', '');
  ...
  function component(object) {
    ...
    if (balansers_with_search == undefined) {
      ...
      network.silent(
        account('http://lampa.isroot.in/lite/withsearch'),
        function (json) {
    ...
    this.startSource = function (json) {
      return new Promise(function (resolve, reject) {
        json.forEach(function (j) {if ( j.balanser === 'alloha' || j.balanser === 'vibix' || j.balanser === 'kinotochka' || j.balanser === 'cdnvideohub' || j.balanser === 'kinopub' ) return;
        ...
        var fin = function fin(call) {
          ...
          network.silent(
            ...
            function (json) {
              ...
              filter_sources = [];
              sources = {};
              json.online.forEach(function (j) {if ( j.balanser === 'alloha' || j.balanser === 'vibix' || j.balanser === 'kinotochka' || j.balanser === 'cdnvideohub' || j.balanser === 'kinopub' ) return;
                var name = balanserName(j);
                sources[name] = {
                  url: j.url,
                  name: j.name,
          ...
          _this5.getFileUrl(
            ...
            function (json, json_call) {
              if (json && json.url) {
                ...
                if (first.url) {
				  var currentProxy = atob('aHR0cHM6Ly9jZHBlYXN5cHJveHkuZHBkbnMub3JnL3Byb3h5L21hbmlmZXN0Lm0zdTg/JnVybD0=');	
					
                  var element = first;
                  element.isonline = true;
                  
				  if (currentProxy !== 'None' && (element.url.indexOf(atob('YWt0ZXItYmxhY2suY29t')) !== -1 || element.url.indexOf(atob('dmt2aWRlbw==')) !== -1)){
						var lastChar = currentProxy.slice(-1);
						...
						if (lastChar === '=') {
							element.url = currentProxy + encodeURIComponent(element.url);

							if (element.url_reserve) {
								element.url_reserve = currentProxy + encodeURIComponent(element.url_reserve);
							}

							if (element.quality) {
								for (var q in element.quality) {
									element.quality[q] = currentProxy + encodeURIComponent(element.quality[q]);
								}
							}
						} else {
							element.url = currentProxy + element.url;

							if (element.url_reserve) {
								element.url_reserve = currentProxy + element.url_reserve;
							}

							if (element.quality) {
								for (var q in element.quality) {
									element.quality[q] = currentProxy + element.quality[q];
								}
							}
						}				  
				  }
  ...
  function startPlugin() {
    ...
    var LAMPAC_FULL_CARD_BUTTON = [
      '<div class="full-start__button selector view--lampa lampac--button" data-subtitle="' +
        manifest.name +
        ' v' +
        manifest.version +
        '">',
      '  <svg width="310" height="304" viewBox="0 0 110 104" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="53" cy="53" r="53" fill="black"/><path d="M81.6744 103.11C98.5682 93.7234 110 75.6967 110 55C110 24.6243 85.3757 0 55 0C24.6243 0 0 24.6243 0 55C0 75.6967 11.4318 93.7234 28.3255 103.11C14.8869 94.3724 6 79.224 6 62C6 34.938 27.938 13 55 13C82.062 13 104 34.938 104 62C104 79.224 95.1131 94.3725 81.6744 103.11Z" fill="white"/><path d="M92.9546 80.0076C95.5485 74.5501 97 68.4446 97 62C97 38.804 78.196 20 55 20C31.804 20 13 38.804 13 62C13 68.4446 14.4515 74.5501 17.0454 80.0076C16.3618 77.1161 16 74.1003 16 71C16 49.4609 33.4609 32 55 32C76.5391 32 94 49.4609 94 71C94 74.1003 93.6382 77.1161 92.9546 80.0076Z" fill="white"/><path d="M55 89C69.3594 89 81 77.3594 81 63C81 57.9297 79.5486 53.1983 77.0387 49.1987C82.579 54.7989 86 62.5 86 71C86 88.1208 72.1208 102 55 102C37.8792 102 24 88.1208 24 71C24 62.5 27.421 54.7989 32.9613 49.1987C30.4514 53.1983 29 57.9297 29 63C29 77.3594 40.6406 89 55 89Z" fill="white"/><path d="M73 63C73 72.9411 64.9411 81 55 81C45.0589 81 37 72.9411 37 63C37 53.0589 45.0589 45 55 45C64.9411 45 73 53.0589 73 63Z" fill="lime"/><polygon points="47,47 47,79 73,63" fill="#2196F3"/></svg>',
      '  <span>LampaFilm</span>',
      '</div>'
    ].join('\n');
  ...
  if (!window.lampac_plugin_akterblack) startPlugin();
})();
