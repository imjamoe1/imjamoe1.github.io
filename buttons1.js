(function() {
    ...
    var Storage = {
        get: function(key, defaultValue) {
            var value = Lampa.Storage.get(CONFIG.STORAGE_PREFIX + key);
        ...
        set: function(key, value) {
            Lampa.Storage.set(CONFIG.STORAGE_PREFIX + key, value);
        },
        remove: function(key) {
            Lampa.Storage.set(CONFIG.STORAGE_PREFIX + key, undefined);
        }
    };
    ...
    function safeExecute(fn, fallback) {
        try {
            ...
            console.error('[Buttons Plugin] Error:', e);
            return fallback;
        }
    }
    ...
    function generateButtonId(button) {
        ...
        // Специальная обработка для кнопки rating
    ...
    function setupRatingButton(button) {
        ...
        // Проверяем, является ли кнопка кнопкой оценки
        var classes = button.attr('class') || '';
        if (classes.indexOf('view--rating') === -1 && classes.indexOf('button--rating') === -1) {
            return;
        }
        ...
        button.off('hover:enter').on('hover:enter', function(e) {
            ...
            // Проверяем, установлена ли уже оценка
            var ratingValue = button.attr('data-rating-value');
            var ratingText = button.find('span').text().trim();
            
            // Если есть цифра оценки, значит оценка уже установлена
            if (ratingValue || (ratingText && /^\d+(\.\d+)?$/.test(ratingText))) {
                // Показываем только иконку, скрываем текст с оценкой
                button.find('span').css('display', 'none');
                
                // Добавляем класс для скрытия оценки
            ...
            // Если оценка не установлена, вызываем оригинальный обработчик
            if (originalClick) {
                // Пытаемся найти оригинальный обработчик
                ...
                // Если не нашли, используем сохраненный
        ...
        // Наблюдаем за изменением текста кнопки (когда ставится оценка)
    ...
    function replaceIcons() {
        ...
        Object.keys(types).forEach(function(type) {
            var selector = '.full-start__button.' + types[type].replace('view--', 'view--').replace('button--', 'button--');
    ...
    function createEditButton() {
        ...
        if (Storage.get('editor_enabled', true) === false) {
            btn.hide();
        }
    ...
    function openEditDialog() {
        ...
        try {
            ...
            function createButtonItem(btn) {
                ...
                // Обработчик для режима отображения
    ...
    if (Lampa.SettingsApi) {
        Lampa.SettingsApi.addParam({
            ...
            onChange: function(value) {
                setTimeout(function() {
                    var currentValue = Storage.get('editor_enabled', true);
                    if (currentValue) {
