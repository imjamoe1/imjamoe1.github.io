(function() {
    ...
    const LAMPA_CACHE_TIME = 24 * 60 * 60 * 1000;
    ...
    // Функция для расчета рейтинга Lampa
    ...
    async function fetchLampaRating(data, card) {
        ...
        // Проверка кэша
        const now = Date.now();
        if (lampaRatingCache[ratingKey] && (now - lampaRatingCache[ratingKey].timestamp < LAMPA_CACHE_TIME)) {
            return lampaRatingCache[ratingKey].value;
        }
        
        try {
            const url = `${LAMPA_RATING_URL}${ratingKey}`;
            ...
            if (Array.isArray(result)) {
                const rating = calculateLampaRating10(result);
