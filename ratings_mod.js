...
(function () {
    ...
    function create() { 
        ...
        this.draw = function (data) { 
            ...
            // Добавляем рейтинги с логотипами
            details.push('<div class="full-start__rate imdb-rating-item">' + 
                '<div>' + imdbRating + '</div>' + 
                '<img src="' + imdbLogoUrl + '" class="rating-logo imdb-logo" alt="IMDB" draggable="false">' + 
                '</div>');
                
            details.push('<div class="full-start__rate tmdb-rating-item">' + 
                '<div>' + vote + '</div>' + 
                '<img src="' + tmdbLogoUrl + '" class="rating-logo tmdb-logo" alt="TMDB" draggable="false">' + 
                '</div>');
            ...
            if (mdblistResult && typeof mdblistResult.tomatoes === 'number' && mdblistResult.tomatoes !== null) { 
                ...
                if (logoUrl) { 
                    details.push('<div class="full-start__rate rt-rating-item">' + 
                        '<div class="rt-score">' + score + '%</div>' + 
                        '<img src="' + logoUrl + '" class="rating-logo rt-logo" alt="RT Status" draggable="false">' + 
                        '</div>'); 
                } 
            } 
    ...
    function startPlugin() {
        if (!window.Lampa || !Lampa.Utils || !Lampa.Lang || !Lampa.Storage || !Lampa.TMDB || !Lampa.Template || !Lampa.Reguest || !Lampa.Api || !Lampa.InteractionLine || !Lampa.Scroll || !Lampa.Activity || !Lampa.Controller) { 
            console.error("Combined Ratings Plugin: Missing Lampa components"); 
            return; 
        }
        ...
        var style_id = 'combined_ratings_style'; 
        if (!$('style[data-id="' + style_id + '"]').length) {
            ...
            <style data-id="${style_id}">
                ...
                /* Additional styles from second plugin */
