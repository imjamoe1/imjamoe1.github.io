if (!window.loaded_modss) {
    ...
    (function() {
        ...
        var TRASH_R = ['$$$####!!!!!!!', '^^^^^^##@', '@!^^!@#@@$$$$$', '^^#@@!!@#!$', '@#!@@@##$$@@'];
        var version_modss = '3.2', API = Protocol() + 'api.lampa.stream/', type = '', jackets = {}, cards, ping_auth, manifest, menu_list = [], vip = false, leftVipD = '', user_id = '', uid = '', IP, logged = false, cashe = encodeURIComponent(Lampa.Base64.encode(window.location.origin));
        ...
        function tryNextUrl() {
            ...
            network.silent(currentUrl, function(json) {
                ...
                console.log('Modss', 'init', '' + currentUrl, network.errorDecode(a, c));
                currentUrlIndex++;
                tryNextUrl();
            }, requestData, {
                dataType: 'text'
