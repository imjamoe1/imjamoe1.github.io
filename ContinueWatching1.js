(function () {
    ...
    // ========================================================================
    // 6. UI: КНОПКА - СОХРАНЯЕТ ПОЗИЦИЮ
    // ========================================================================

    var continueButtonInserted = false;
    ...
    function createContinueButton(movie, params) {
        ...
        var continueButtonHtml = `
            <div class="full-start__button selector button--continue-watch" data-stable-id="continue_watch_button" style="margin-top: 0;">
        ...
        var btn = $(continueButtonHtml);
        btn.on('hover:enter', function () { handleContinueClick(movie, this); });
        return btn;
    }

    function updateContinueButton(movie, params) {
        var container = $('.full-start-new__buttons, .full-start__buttons').first();
        if (!container.length) return;

        var existingBtn = container.find('.button--continue-watch');
        ...
        if (existingBtn.length) {
            ...
            // Обновляем текст
            existingBtn.find('div').html(labelText);
            
            // Обновляем прогресс
            var circle = existingBtn.find('circle');
            if (circle.length) {
                circle.attr('stroke-dasharray', dashArray + ' 65.97');
            }
            
            // Обновляем обработчик
            existingBtn.off('hover:enter');
            existingBtn.on('hover:enter', function () { handleContinueClick(movie, this); });
    ...
    function insertContinueButton(render, movie, params) {
        ...
        var existingBtn = buttonsContainer.find('.button--continue-watch');
        if (existingBtn.length) {
            // Обновляем существующую кнопку
            updateContinueButton(movie, params);
            return;
        }

        // Создаём новую кнопку
        var continueBtn = createContinueButton(movie, params);

        // Ищем кнопку Lumio (view--nexus)
        var lumioBtn = buttonsContainer.find('.view--nexus');
        
        if (lumioBtn.length) {
            // Вставляем перед Lumio
            lumioBtn.before(continueBtn);
        } else {
            // Ищем кнопку Torrent
            var torrentBtn = buttonsContainer.find('.view--torrent').last();
        ...
        Lampa.Listener.follow('full', function (e) {
            if (e.type === 'complite') {
                requestAnimationFrame(function () {
                    ...
                    if (!params) {
                        // Если нет истории, удаляем кнопку если есть
                        var existingBtn = render.find('.button--continue-watch');
                        if (existingBtn.length) {
                            existingBtn.remove();
                        }
                        return;
                    ...
                    // Вставляем или обновляем кнопку
                    insertContinueButton(render, e.data.movie, params);
                });
            }
        });
    }
    ...
    function add() {
        ...
        migrateOldData();
        console.log("[ContinueWatch] v73 Loaded. Button position is now preserved.");
    }
