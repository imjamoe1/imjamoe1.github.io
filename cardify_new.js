(function () {
    ...
    // ============ ДОБАВЛЯЕМ ЭФФЕКТ РАЗМЫТИЯ КАК В APPLECATION ============
    function addApplecationBlurEffect() {
        ...
        // Добавляем стили для размытого фона
        const blurStyles = `<style>
            /* Фон с размытием как в Applecation */
            .cardify__background {
                left: 0 !important;
                opacity: 0 !important;
            ...
            .cardify__background.dim {
                filter: blur(30px);
            }

            .cardify__background.loaded.applecation-animated {
                opacity: 1 !important;
            }

            /* Эффект размытия при скролле */
            .cardify__background.scroll-blur {
                transition: filter 0.4s cubic-bezier(0.4, 0, 0.2, 1), 
                            opacity 0.4s cubic-bezier(0.4, 0, 0.2, 1);
            }

            /* Убираем стандартную маску Lampa */
            ...
            body.advanced--animation:not(.no--animation) .cardify__background.loaded {
                animation: none !important;
            }

            /* Градиентный оверлей для лучшей читаемости */
            .cardify__background::after {
                content: '';
                position: absolute;
                top: 0;
                left: 0;
            ...
            /* Улучшаем отображение контента на размытом фоне */
            ...
            .cardify__background.parallax {
                transform: translateY(var(--scroll-offset, 0px));
                transition: transform 0.8s cubic-bezier(0.4, 0, 0.2, 1);
            }
        </style>`;

        Lampa.Template.add('cardify_blur_css', blurStyles);
        $('body').append(Lampa.Template.get('cardify_blur_css', {}, true));

        // Функция для добавления эффекта размытия при скролле
        function addScrollBlurEffect() {
            Lampa.Listener.follow('full', (event) => {
                if (event.type === 'complite') {
                    const activity = event.object.activity;
                    const render = activity.render();
                    const background = render.find('.cardify__background');
                    const content = render.find('.full-start-new__body');
                    
                    if (!background.length || !content.length) return;

                    // Добавляем классы для управления эффектами
                    background.addClass('scroll-blur parallax');

                    // Функция обновления эффектов при скролле
                    const updateScrollEffects = () => {
                        const scrollTop = content.scrollTop() || 0;
                        const maxScroll = content[0].scrollHeight - content[0].clientHeight;
                        
                        // Нормализуем значение прокрутки (0-1)
                        const scrollNormalized = maxScroll > 0 ? Math.min(1, scrollTop / maxScroll) : 0;
                        
                        // Интенсивность размытия (0-30px)
                        const blurValue = scrollNormalized * 30;
                        
                        // Прозрачность (100%-30%)
                        const opacityValue = 1 - (scrollNormalized * 0.7);
                        ...
                        background.css({
                            ...
                            '--scroll-offset': `${parallaxValue}px`
                        });

                        // Затемняем градиент при прокрутке
                        const gradientOpacity = 0.8 + (scrollNormalized * 0.2);
                        background.css('--gradient-opacity', gradientOpacity);
                    };
                    ...
                    const waitForBackgroundLoad = (callback) => {
                        if (!background.length) {
                            callback();
                            return;
                        }

                        // Если фон уже загружен
                    ...
                    waitForBackgroundLoad(() => {
                        // После загрузки запускаем начальные эффекты
                        updateScrollEffects();
                    });

                    // Очистка при уничтожении активности
                    Lampa.Listener.follow('activity', (destroyEvent) => {
                        if (destroyEvent.type === 'destroy' && destroyEvent.object.activity === activity) {
                            content.off('scroll', onScroll);
                            background.removeClass('scroll-blur parallax');
                        }
                    });
                }
            });
        ...
        // Модифицируем функцию skodf для добавления размытия
        const originalSkodf = Follow.skodf;
        Follow.skodf = function(e) {
            // Вызываем оригинальную функцию
            originalSkodf.call(this, e);
            
            // Добавляем дополнительные эффекты
            const background = e.object.activity.render().find('.full-start__background');
            if (background.length) {
                // Ждем немного и запускаем эффекты
                setTimeout(() => {
                    addScrollBlurEffect();
                }, 100);
            }
    ...
    function startPlugin() {
        ...
        // Добавляем эффект размытия Applecation
        addApplecationBlurEffect();
    ...
    `;
        ...
        Lampa.Listener.follow(Type.de([102, 117, 108, 108]), function (e) {
            if (Type.co(e)) {
                Follow.skodf(e);
    ...
    if (Follow.go) startPlugin(); else {
        Lampa.Listener.follow(Type.de([97, 112, 112]), function (e) {
