(function () {
  ...
  style.textContent = `
    ...
    .ani_svg img {
      ...
      max-height: 100%;
      object-fit: contain;
      filter: hue-rotate(30deg); /* Пример изменения цвета */
    }
    .ani_svg.focus {
    ...
    .activity__loader_prv {
      background-color: #FF8C00; /* Установлен цвет */
    }
  `;
  ...
  function insert_activity_loader(src) {
    ...
    activity_loader_style.textContent = `
      .activity__loader {
        background: url(${src}) #FF8C00 no-repeat center center;
      }
      .activity__loader_prv {
        background: url(${src}) #FF8C00 no-repeat center center;
      }
    ...
    return `
      <div class="ani_svg selector" tabindex="0">
        <picture>
          ...
          <img src="${src}" style="visibility:visible; max-width:100%; fill:#FF8C00">
        </picture>
      </div>
    `;
  }
